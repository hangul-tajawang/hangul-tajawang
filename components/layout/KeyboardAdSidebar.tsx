...
export const KeyboardAdSidebar: React.FC = () => {
  ...
  if (isCollapsed) {
    return (
      ...
      >
        ...
        <span className="text-primary font-bold text-xs tracking-[0.3em] [writing-mode:vertical-rl] uppercase">
          스폰서 광고 보기
        </span>
      </div>
    );
  ...
  return (
    <div className="w-[300px] flex flex-col gap-6 relative bg-surface-low/30 p-2 -mx-2 rounded-2xl transition-all duration-500">
      <button 
        onClick={() => setIsCollapsed(true)}
        className="absolute top-3 right-3 flex items-center gap-1.5 px-3 py-1.5 bg-surface-lowest border border-outline-variant text-zinc-500 hover:text-zinc-800 hover:bg-surface-high rounded-full transition-all shadow-sm z-10 group"
      >
        <span className="text-[10px] font-bold tracking-widest">접어두기</span>
        <X size={12} className="group-hover:rotate-90 transition-transform" />
      </button>
      
      <div className="flex flex-col px-2 pt-2">
        <span className="text-primary font-bold text-[10px] uppercase tracking-[0.2em] mb-2 flex items-center gap-1.5">
