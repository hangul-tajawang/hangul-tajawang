      ...
      <div className="mb-8">
        {allDone ? (
          <div className="flex items-center gap-3 px-6 py-4 bg-green-50 border border-green-200 rounded-2xl">
            ...
            <p className="font-bold text-green-700">완간을 새기셨습니다. 이 책은 내 서재에 꽂혀 있어요.</p>
          </div>
        ) : nextEpisode ? (
          <Link
            prefetch={false}
            href={`/transcription/${nextEpisode.id}`}
            className="inline-flex items-center gap-3 px-8 py-4 primary-gradient text-white font-bold rounded-full shadow-xl shadow-primary/20 hover:scale-105 transition-transform"
          >
        ...
        {episodes.map((ep) => {
          ...
          return (
            <li key={ep.id}>
              ...
              >
                <span className={`w-8 h-8 shrink-0 rounded-full flex items-center justify-center text-xs font-bold ${done ? "bg-green-100 text-green-600" : "bg-surface-high text-zinc-400"}`}>
                ...
                {progressing && (
                  <span className="shrink-0 flex items-center gap-1 px-2.5 py-1 bg-primary/10 text-primary text-[10px] font-bold rounded-full"><Feather size={11} /> {Math.round(inProgressPercent)}% 저장됨</span>
                )}
