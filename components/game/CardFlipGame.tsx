  ...
  const gameOverModal = gameState === "gameover" && (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 sm:p-6">
        ...
        <div className="relative max-w-lg w-full max-h-[90vh] overflow-y-auto no-scrollbar bg-white rounded-2xl p-8 shadow-2xl text-center border border-zinc-200 animate-in zoom-in duration-500">
            ...
            <p className="text-zinc-500 font-bold mb-10">{timeLeft > 0 ? "놀라운 기억력입니다!" : "시간이 다 되었습니다. 다시 도전해보세요!"}</p>
            ...
            ) : <div className="mb-6 text-sm font-bold text-green-600 animate-pulse">랭킹에 기록이 성공적으로 등록되었습니다!</div>}
            

            <div className="mb-6 flex justify-center empty:hidden">
              <AdSenseUnit label="content-banner-mobile" width={320} height={100} tight />
            </div>
            <button onClick={initGame} className="w-full py-5 bg-zinc-900 text-white text-xl font-bold rounded-2xl hover:scale-[1.02] active:scale-95 transition-all shadow-xl flex items-center justify-center gap-3"><RotateCcw size={24} /> 다시 시작하기</button>
        </div>
    </div>
  );
      ...
      {gameState === "ready" && (
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm z-30 flex items-center justify-center p-4">
              <div className="bg-white p-10 rounded-2xl shadow-2xl flex flex-col items-center gap-8 max-w-sm w-full border border-zinc-200">
                  ...
                  <div className="text-center"><h3 className="text-3xl font-bold text-zinc-900 mb-2 leading-tight">기억력 타자</h3><p className="text-zinc-500 text-sm font-medium">카드 뒷면의 단어를 치면 뒤집힙니다.<br/>기억력을 발휘해 짝을 맞추세요!</p></div>
                  <button onClick={initGame} className="w-full py-5 bg-purple-600 hover:bg-purple-700 text-white text-xl font-bold rounded-2xl transition-all shadow-xl shadow-purple-200">게임 시작</button>
              </div>
          </div>
      )}
  ...
  if (isMobilePlaying && overlay) {
    ...
    const mobileHud = (
      <>
        ...
        <span className="flex items-center gap-1"><span className="text-[9px] text-zinc-500 font-bold uppercase">짝</span><span className="text-sm font-bold text-green-400 tabular-nums">{cards.filter(c => c.isMatched).length / 2}/8</span></span>
    ...
    <div className="w-full max-w-6xl mx-auto flex flex-col gap-4 py-2 animate-in fade-in duration-700 h-[calc(100vh-100px)] max-h-[850px] min-h-[650px]">
      ...
      <div className="w-full flex justify-between items-center px-8 py-4 bg-zinc-900 text-white rounded-2xl shadow-xl border border-zinc-800 shrink-0">
        ...
        <div className="flex items-center gap-6">
            ...
            <div className="text-right hidden sm:block"><div className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest leading-tight">Memory Mode</div><div className="font-bold text-zinc-300 text-sm leading-tight italic">기억력 타자</div></div>
        </div>
      </div>
      ...
      <div className="w-full flex flex-col lg:flex-row gap-4 flex-1 overflow-hidden min-h-0">
        ...
        <div className="w-full lg:w-72 bg-white rounded-2xl border border-zinc-200 p-6 shadow-lg flex flex-col shrink-0">
            <div className="flex items-center gap-2 mb-6"><Trophy className="text-yellow-500" size={20} /><h3 className="text-lg font-bold">기억력 랭킹</h3></div>
            <div className="flex-1 space-y-3 overflow-y-auto pr-1 custom-scrollbar">
                ...
                rankings.length > 0 ? rankings.map((rank, i) => (
                    <div key={i} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0"><p className="text-sm font-bold truncate text-zinc-900 leading-tight">{rank.profiles?.nickname || '익명'}</p><p className="text-[9px] font-bold text-zinc-400">Combo {rank.max_combo}</p></div>
