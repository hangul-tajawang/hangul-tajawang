...
export const ManuscriptSubmit: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 md:px-6 pb-16">
      {/* 약속 */}
      <div className="bg-surface-lowest rounded-2xl p-6 md:p-8 mb-6 border border-surface-high">
        <h2 className="text-lg font-bold mb-5 flex items-center gap-2"><Feather size={18} className="text-primary" /> 선정되면 이렇게 됩니다</h2>
      ...
      {/* 투고 방법 */}
      <div className="bg-surface-lowest rounded-2xl p-6 md:p-8 mb-6 border border-surface-high">
        <h2 className="text-lg font-bold mb-5 flex items-center gap-2"><Mail size={18} className="text-primary" /> 투고 방법</h2>
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <button onClick={copyEmail} className="shrink-0 px-4 py-3 bg-surface-low rounded-xl text-sm font-bold hover:text-primary transition-colors flex items-center gap-1.5">
        ...
        <div className="text-xs text-zinc-500 leading-relaxed space-y-1.5 mb-6">
          <p className="font-bold text-zinc-600">메일에 담아주세요</p>
        ...
        <div className="mb-6 rounded-2xl border border-surface-high bg-surface-low overflow-hidden">
          <div className="flex items-center justify-between px-4 py-2.5 border-b border-surface-high">
            <span className="text-xs font-bold text-zinc-500">메일 양식 미리보기</span>
            <button onClick={copyForm} className="text-xs font-bold text-primary flex items-center gap-1 hover:underline underline-offset-2">
              {formCopied ? <><Check size={12} className="text-green-600" /> 복사됨</> : <><Copy size={12} /> 양식 복사</>}
            </button>
          </div>
          <pre className="px-4 py-3.5 text-xs leading-relaxed text-zinc-600 whitespace-pre-wrap font-medium">{`제목: ${MAIL_SUBJECT}\n\n${MAIL_BODY}`}</pre>
        </div>

        <a href={mailtoHref} className="block w-full py-4 primary-gradient text-white text-center font-bold rounded-2xl shadow-xl shadow-primary/20 hover:scale-[1.02] transition-transform">
          ✉️ 양식이 채워진 메일로 투고하기
        </a>
