  ...
  return (
    <div className="w-full max-w-4xl mx-auto py-10 px-4 md:px-6 text-on-surface">
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-black flex items-center gap-2"><BookOpen size={18} className="text-primary" /> 책 {allBooks.length}권</h2>
        <Link href="/adminsangwon/book" className="inline-flex items-center gap-1.5 px-4 py-2 primary-gradient text-white text-sm font-black rounded-full hover:scale-105 transition-transform">
          <Plus size={15} /> 새 책 게재
        </Link>
      </div>
      ...
      {totalPages > 1 && (
        <nav className="mb-12 -mt-6 flex items-center justify-center gap-2" aria-label="책 목록 페이지">
          {page > 1 && <Link href={`/adminsangwon?page=${page - 1}`} className="px-4 py-2 rounded-full bg-surface-high text-sm font-black text-zinc-500 hover:text-primary">← 이전</Link>}
          <span className="text-sm font-black text-zinc-500">{page} / {totalPages}</span>
          {page < totalPages && <Link href={`/adminsangwon?page=${page + 1}`} className="px-4 py-2 rounded-full bg-surface-high text-sm font-black text-zinc-500 hover:text-primary">다음 →</Link>}
        </nav>
      )}

      {/* 작가 목록 */}
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-black flex items-center gap-2"><UserRound size={18} className="text-primary" /> 작가 {authors.length}명</h2>
        <Link href="/adminsangwon/author" className="inline-flex items-center gap-1.5 px-4 py-2 primary-gradient text-white text-sm font-black rounded-full hover:scale-105 transition-transform">
      ...
      <ul className="space-y-2">
        {authors.map((a) => (
          <li key={a.id} className="flex items-center gap-4 px-5 py-4 rounded-2xl border border-surface-high bg-surface-lowest">
            <div className="flex-1 min-w-0">
              <p className="font-black truncate">{a.name} <span className="ml-2 text-xs font-bold text-zinc-400">{a.id}</span></p>
