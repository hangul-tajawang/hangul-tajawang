...
export default async function SeriesPage({ params }: Props) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto py-10 md:py-16 px-4 md:px-6 text-on-surface">
      ...
      {/* 표지 헤더 */}
      <header className="relative overflow-hidden rounded-2xl md:rounded-2xl bg-gradient-to-br from-rose-700 to-rose-950 text-white px-6 md:px-12 py-10 md:py-14 mb-10 flex flex-col md:flex-row items-center gap-8 md:gap-12">
        ...
        <div className="flex-1 text-center md:text-left">
          <p className="text-rose-200 font-bold text-[11px] uppercase tracking-[0.3em] mb-4">한글타자왕 오리지널 연재</p>
          <h1 className="font-serif font-bold text-4xl md:text-5xl mb-3 break-keep">{series.title}</h1>
          ...
          <div className="mt-7 flex flex-wrap justify-center md:justify-start gap-2 text-[11px] font-bold">
      ...
      {/* 소개 */}
      <p className="text-zinc-600 leading-loose mb-10 break-keep">{series.description}</p>
      ...
      {(series.authorProfile || series.authorId) && (
        <div className="mb-10 flex items-center gap-4 p-5 rounded-2xl border border-surface-high bg-surface-lowest">
          ...
          <div className="flex-1 min-w-0">
            {series.authorId ? (
              <Link prefetch={false} href={`/authors/${series.authorId}`} className="font-bold hover:text-primary transition-colors">
                {series.author} <span className="text-xs font-bold text-zinc-400">작품 더 보기 →</span>
              </Link>
            ) : (
              <p className="font-bold">{series.author}</p>
            )}
          ...
          <div className="flex gap-2 shrink-0">
            {series.authorProfile?.sns && (
              <a href={series.authorProfile.sns} target="_blank" rel="noopener noreferrer" className="px-3.5 py-2 rounded-full bg-surface-high text-xs font-bold text-zinc-600 hover:text-primary transition-colors">
                SNS
              </a>
            )}
            {series.authorProfile?.blog && (
              <a href={series.authorProfile.blog} target="_blank" rel="noopener noreferrer" className="px-3.5 py-2 rounded-full bg-surface-high text-xs font-bold text-zinc-600 hover:text-primary transition-colors">
                블로그
              </a>
            )}
          </div>
      ...
      {/* 목차 + 진행 (클라이언트) */}
      <h2 className="text-xl font-bold mb-5">목차</h2>
      ...
      <div className="mt-14 pt-8 border-t border-surface-high text-center">
        ...
        <Link prefetch={false} href="/transcription" className="text-sm font-bold text-zinc-400 hover:text-primary transition-colors">← 다른 작품 필사하러 가기</Link>
      </div>
    </div>
  );
}
