...
export function generateStaticParams() {
  return PILSA_SERIES.map((s) => ({ seriesId: s.id }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { seriesId } = await params;
  const series = PILSA_SERIES.find((s) => s.id === seriesId);
  if (!series) return {};
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `https://www.hangul-tajawang.com/images/book-covers/${seriesId}.jpg`,
          width: 900,
          height: 1200,
          alt: `${series.title} 표지`,
        },
...
export default async function SeriesPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    image: `https://www.hangul-tajawang.com/images/book-covers/${seriesId}.jpg`,
    url: `https://www.hangul-tajawang.com/transcription/series/${seriesId}`,
  };
    ...
    <div className="w-full max-w-3xl mx-auto py-10 md:py-16 px-4 md:px-6 text-on-surface">
      ...
      <header className="relative overflow-hidden rounded-[2rem] md:rounded-[3rem] bg-gradient-to-br from-rose-700 to-rose-950 text-white px-6 md:px-12 py-10 md:py-14 mb-10 flex flex-col md:flex-row items-center gap-8 md:gap-12">
        <div className="w-44 md:w-56 shrink-0">
          <BookCoverArt seriesId={series.id} title={series.title} author={series.author} cover={series.cover} priority className="shadow-2xl" />
        </div>
        <div className="flex-1 text-center md:text-left">
          ...
          <div className="mt-7 flex flex-wrap justify-center md:justify-start gap-2 text-[11px] font-black">
            <span className="px-3 py-1.5 bg-white/15 rounded-full flex items-center gap-1.5">
              <CheckCircle2 size={13} /> {episodes.length >= series.totalEpisodes ? '완결' : `연재 중 · ${episodes.length}화까지 공개`}
            </span>
      ...
      <SeriesTOC episodes={episodes.map((e) => ({ id: e.id, title: e.title, episode: e.episode || 0, wordCount: e.wordCount }))} />
